...
if [ -e "${WORKDIR}/nezha.sh" ] && [ -e "${WORKDIR}/config.json" ] && [ -e "${WORKDIR}/argo.sh" ]; then
  ...
  (crontab -l | grep -F "@reboot pkill -kill -u $(whoami) && ${CRON_NEZHA} ${CRON_SB} ${CRON_ARGO}") || \
    (crontab -l; echo "@reboot pkill -kill -u $(whoami) && ${CRON_NEZHA} ${CRON_SB} ${CRON_ARGO}") | crontab -
  (crontab -l | grep -F "pgrep -x \"npm\" > /dev/null || ${CRON_NEZHA}") || \
    (crontab -l; echo "*/10 * * * * pgrep -x \"npm\" > /dev/null || ${CRON_NEZHA}") | crontab -
  (crontab -l | grep -F "pgrep -x \"web\" > /dev/null || ${CRON_SB}") || \
    (crontab -l; echo "*/10 * * * * pgrep -x \"web\" > /dev/null || ${CRON_SB}") | crontab -
  (crontab -l | grep -F "pgrep -x \"bot\" > /dev/null || ${CRON_ARGO}") || \
  ...
  (crontab -l | grep -F "@reboot pkill -kill -u $(whoami) && ${CRON_ARGO}") || \
