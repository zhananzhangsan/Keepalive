...
argo_configure() {
  ...
  if [[ -z $ARGO_AUTH || -z $ARGO_DOMAIN ]]; then
    ...
    if [[ "$argo_choice" == "y" || "$argo_choice" == "Y" ]]; then
      ...
      reading "请输入 argo 固定隧道名称，区别大小写: " ARGO_NAME
      green "你的 argo 固定隧道密钥为: $ARGO_NAME"
...
EOF
    ...
    args="tunnel --edge-ip-version auto --no-autoupdate --protocol http2 run --token ${ARGO_AUTH} --tunnel ${ARGO_NAME} --hostname ${ARGO_DOMAIN} --url http://localhost:${vmess_port}"
