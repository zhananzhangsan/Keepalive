...
async function runRemoteCommand(server, SCRIPT_URL) {
  ...
  const payload = {
    ...
    username: SSH_USER,
    password: SSH_PASS,
    command: `VMESS_PORT=${VMESS_PORT} HY2_PORT=${HY2_PORT} SOCKS_PORT=${SOCKS_PORT} SOCKS_USER=${SOCKS_USER} SOCKS_PASS='${SOCKS_PASS}' ARGO_DOMAIN=${ARGO_DOMAIN} ARGO_AUTH='${ARGO_AUTH}' NEZHA_SERVER=${NEZHA_SERVER} NEZHA_PORT=${NEZHA_PORT} NEZHA_KEY=${NEZHA_KEY} bash <(curl -Ls ${SCRIPT_URL})`
  };
